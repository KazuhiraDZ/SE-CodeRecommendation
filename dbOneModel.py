...
def insert(path,class_prediction_path,method_prediction_path):
	print(path)
	start = time.time()
	conn = mdb.connect(host='127.0.0.1', port=3306, user='root', passwd='fdse', db='dataset', charset='utf8')
	cursor = conn.cursor()
	count = 0
	values = []
	...
	for serialized_example in tf.python_io.tf_record_iterator(path):
		...
		if data.size() <= 90 and data.get_child_num(data) == 0:
		...
		if count % 1000 == 0:
			try:
				cursor.executemany('INSERT INTO dataset(data,classPrediction,methodPrediction,trace,block,holeSize,variables,originalStatement,noise_label) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)',values)
				conn.commit()
			except:
			    import traceback
			    traceback.print_exc()
			    ...
			    for value in values:
			    	try:
			    		cursor.execute('INSERT INTO dataset(data,classPrediction,methodPrediction,trace,block,holeSize,variables,originalStatement,noise_label) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)',value)
			    		conn.commit()
			    	except:
			    		conn.rollback()
			    		print (value[7])
...
def select(start,end):
	...
	variables = []
	try:
		cursor.execute("select data,variables from dataset where `index` > " + str(start) + " limit " + str(end))
		...
		gold_labels = []
		variables = []
		return data,gold_labels,variables
		#conn.rollback()
	finally:
		cursor.close()
		conn.close()
