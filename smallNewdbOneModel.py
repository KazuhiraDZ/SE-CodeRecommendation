...
def select_for_sequence(start,end):
	conn = mdb.connect(host='127.0.0.1', port=3306, user='root', passwd='fdse', db='smalldataset', charset='utf8')
	cursor = conn.cursor()
	data = []
	gold_labels = []
	...
	try:
		...
		for i in xrange(numrows):
			...
			#	variable = list(reversed(variable))[:10]
			tree = pickle.loads(row[0])
			tree = tree.depth_first(tree,vocab)          
			data.append(tree)
			gold_labels.append(tree.gold_label)
			#gold_labels.append(tree.gold_label)
			#variables.append(variable)
...
# for i in  xrange(10):
# 	for j in xrange(5):
# 		file_path = '/home/x/mydisk/smallnewestrecords/tree'+str(i+1)+'_'+str(j+1)+'.tfrecords'
# 		class_prediction_path = '/home/x/mydisk/smallnewestrecords/classPrediction'+str(i+1)+'_'+str(j+1)+'.txt'
# 		method_prediction_path = '/home/x/mydisk/smallnewestrecords/methodPrediction'+str(i+1)+'_'+str(j+1)+'.txt'
# 		start = time.time()
